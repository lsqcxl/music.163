...
class RecommendMusic extends React.Component {
	...
	componentWillMount() {
		var that = this
		Fetch.Get("./json/songMenu.json", {}).then((res) => {
			return res.json()
		}).then((json) => {
			that.setState({
				songs: json.result
		...
		var that = this
		Fetch.Get("./json/newsong.json", {}).then((res) => {
			return res.json()
		}).then((json) => {
			that.setState({
	...
	showsongs() {
		...
		if(songs.length) {
			songs.forEach((val, i) => {
				let aa = (val.playCount / 10000).toFixed(1)
				arr.push(
				<a className="rem-li" href={"#/playlist/"+i}>
	    				<div className="rem-img">
	    					<img className ="u-img" src={val.picUrl}/>
	    					<span className="u-num">{aa}万</span>
	    				</div>
	    				<p className="rem-name">{val.name}</p>
    				</a>)
			})
		}
		return arr
	...
	newsongs() {
		...
		let arr = []
		if(newsongs.length) {
			newsongs.forEach((val, i) => {
				arr.push(
					<Link to={"/song/"+val.id} className="rec-item">
						<div className ="rec-item_top">
							<div className="sgch">
								<div className="rec-thide">{val.name}</div>
								<div className="rec-ingf">
									<i className="yin"></i>
							...
							<div className="sgchfr">
								<span className="u-hmsprt sgchply">
								</span>
							</div>
						</div>
					</Link>
			...
			<div className="m-tabct">
				...
				<div className="tabctitem">
					<div className="rec-top">
						<h2 className="rec-tl">推荐歌单</h2>
						<div className="rec-songs">
							<div className="rem-ul">
								{this.showsongs()}
							</div>
						</div>
						<h2 className="rec-tl">最新音乐</h2>
						<div className="rec-newsong">
							<div className = "rec-mg">
								{this.newsongs()}
							</div>
						</div>
					</div>
				...
				<div className="re-footer">
					<div className="rec-wrap">
						...
						< a href="#/applink"><div className="openapp">
							打开app,发现更多好音乐
						</div></a>
